...
const spine_zig = @import("spine_zig");
...
pub fn main(init: std.process.Init) !void {
    ...
    var node = try spine_zig.Node.init("common", "spine-zig-bench-node", io, allocator);
    defer node.deinit();
...
fn benchPubSub(node: *spine_zig.Node, io: std.Io) !void {
...
fn benchServiceCall(node: *spine_zig.Node, io: std.Io) !void {
...
fn serviceCallWorker(caller: *spine_zig.ServiceCaller(u32, u32), iterations: u64) void {
    var i: u64 = 0;
    while (i < iterations) : (i += 1) {
...
fn benchServiceCallParallel(node: *spine_zig.Node, io: std.Io) !void {
